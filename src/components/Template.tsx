...
const Template = () => {
  ...
  const images = ["/museum.jpeg", "/modern.png"];
  const names = ["Museum", "Modern"];
  ...
  return (
    <div>
      <div>
        {!addContract ? (
          <div>
            <h1 className="font-bold text-xl">Select template</h1>
              ...
              {images.map((image, index) => (
                <div key={index} onClick={() => setSelectedImage(image)}>
                  <Image
                    ...
                    height={200}
                    alt="Option"
                    className={`rounded-xl m-4 cursor-pointer border-4 ${
                      selectedImage === image
                        ? "border-[#5d50b4]"
                        : "border-transparent"
                    }`}
